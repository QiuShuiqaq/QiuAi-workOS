...
function buildSingleMediaAssistantWorkflowSteps(input: {
  mediaLabel: string;
  ...
  return [
    {
      ...
      type: 'input',
      name: '接收创作需求',
      instruction: `接收用户输入、可选参考图、比例和风格要求，确认本次只生成一个${input.mediaLabel}结果。`
    },
    {
      id: 'generate_media',
      order: 2,
      ...
      instruction: `调用对应媒体生成模型，输出一个远程${input.artifactLabel} URL，不返回二进制或 base64。`
    },
    {
      id: 'save_media',
      order: 3,
      type: 'tool',
      name: `保存${input.artifactLabel}`,
      instruction: `把模型返回的远程${input.artifactLabel}下载保存到 PC 本地产物目录。`,
      toolIds: ['local-filesystem']
    },
    {
      id: 'final_output',
      ...
      type: 'output',
      name: '返回结果',
      instruction: `返回生成结果、本地${input.artifactLabel}路径和需要人工复核的事项。`
    }
  ];
}
...
function buildSingleMediaAssistantWorkflowGraph(input: {
  ...
  return {
    ...
    edges: [
      graphEdge('start', 'receive_input'),
      graphEdge('receive_input', 'generate_media'),
      graphEdge('generate_media', 'save_media')
    ],
    runtimePolicy: {
...
const freeBasicRoleTemplates: BaseServerRoleTemplateCatalogEntry[] = [
  ...
  }),
  {
    ...
    description: '面向个人和企业的单次生图数字员工。用户只需要描述画面、比例和用途，或上传一张参考图，即可生成一张可预览、可下载的图片产物。',
    recommendedPlanCode: 'PERSONAL_FREE',
    businessGoal: '把零散生图需求收敛成一个简单入口，适合头像、封面、海报草图、商品素材和创意图片的单次生成。',
    knowledgeSources: ['用户提示词', '可选参考图片', '企业视觉规范', '历史优质提示词'],
    tools: ['local-filesystem'],
    executionProfile: createConversationOfficeExecutionProfile({
      summary: '由用户输入图片需求或上传参考图，调用生图模型生成一张图片并保存到本地。',
    ...
    skills: [
      skill('single_image_prompting', '单图提示词理解', '把用户的画面、风格、比例和用途要求整理成一次生图指令。'),
      skill('reference_image_generation', '参考图生成', '在用户上传参考图时保留主体关键外观、颜色、结构和风格方向。'),
      skill('image_result_delivery', '图片产物保存', '把远程图片结果保存为本地 PNG，方便预览、复制和后续使用。')
    ],
    workflowSteps: buildSingleMediaAssistantWorkflowSteps({
      mediaLabel: '图片',
      artifactLabel: 'PNG 图片'
    }),
  ...
  },
  {
    ...
    tools: ['local-filesystem'],
    executionProfile: createConversationOfficeExecutionProfile({
      summary: '由用户输入视频需求或上传参考图，调用生视频模型生成一条短视频并保存到本地。',
      inputSources: ['chat', 'uploaded_files', 'enterprise_knowledge'],
      toolCapabilities: ['llm', 'local_files', 'approval_queue'],
    ...
    skills: [
      skill('single_video_prompting', '单条视频提示词理解', '把用户的镜头、动作、画幅、时长和风格要求整理成一次生视频指令。'),
      skill('reference_image_video_generation', '参考图生视频', '在用户上传参考图时保留主体外观和画面风格，生成稳定短视频。'),
      skill('video_result_delivery', '视频产物保存', '把远程视频结果保存为本地 MP4，方便预览、打开和复核。')
    ],
    workflowSteps: buildSingleMediaAssistantWorkflowSteps({
      mediaLabel: '视频',
      artifactLabel: 'MP4 视频'
    }),
    workflowGraph: buildSingleMediaAssistantWorkflowGraph({
      mediaKind: 'video',
      mediaLabel: '视频',
