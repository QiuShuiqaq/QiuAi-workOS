...
export type ToolActionFieldType = 'text' | 'number' | 'textarea' | 'boolean';

export interface ToolActionPort {
  key: string;
  label: string;
  type: ToolValueType;
  required?: boolean;
...
export interface ToolActionUiField {
  key: string;
  label: string;
  placeholder?: string;
  type?: ToolActionFieldType;
  format?: 'text' | 'json';
}
...
export interface ToolActionDefinition {
  ...
  category: ToolActionCategory;
  description: string;
  input: ToolActionPort[];
  output: ToolActionPort[];
...
export interface ToolActionHealth {
  toolId: string;
  actionId: string;
  name: string;
  category: ToolActionCategory;
  status: ToolActionStatus;
  inputTypes: ToolValueType[];
  outputTypes: ToolValueType[];
  requiredConfig: string[];
...
export interface ToolActionCatalog {
  packages: ToolPackageDefinition[];
  actions: ToolActionDefinition[];
}

export interface ListToolActionCatalogResponse {
  data: ToolActionCatalog;
}

export const qiuaiToolPackages: ToolPackageDefinition[] = [
  {
    id: 'web-search',
    name: '网页工具',
    category: 'web',
    description: '网页搜索和网页正文读取。'
  },
  {
    id: 'office-document',
    name: '办公文档',
    category: 'document',
    description: '读取文档文本，生成 Word、Excel、CSV 和 PPT。'
  },
  {
    id: 'local-filesystem',
    name: '本地文件',
    category: 'file',
    description: '读取、写入和列出本地授权文件。'
  },
  {
    id: 'video-processing',
    name: '视频处理',
    category: 'video',
    description: '读取视频信息、抽取关键帧和导出剪辑视频。'
  },
  {
    id: 'browser-automation',
    name: 'RPA 浏览器',
    category: 'integration',
    description: '在 PC 本机打开网页、执行基础点击填表和页面文本提取。'
  },
  {
    id: 'http-request',
    name: 'HTTP 接口',
  ...
  },
  {
    ...
    name: 'MCP 工具',
    category: 'integration',
    description: '调用本地或远程 MCP 工具。'
  }
];

export const qiuaiToolActions: ToolActionDefinition[] = [
  {
  ...
  },
  {
    ...
    output: [
      { key: 'artifact', label: 'Word 文件', type: 'artifact' }
    ],
    defaultInput: {
      title: '{{task.title}}',
      folder: 'documents',
  ...
  },
  {
    ...
    output: [
      { key: 'artifact', label: 'Markdown 文件', type: 'artifact' }
    ],
    defaultInput: {
      title: '{{task.title}}',
      folder: 'documents',
  ...
  },
  {
    ...
    output: [
      { key: 'artifact', label: 'PPT 文件', type: 'artifact' }
    ],
    defaultInput: {
      title: '{{task.title}}',
  ...
  },
  {
    ...
    output: [
      { key: 'artifact', label: '文本文件', type: 'artifact' }
    ],
  ...
  },
  {
    ...
    input: [
      { key: 'path', label: '目录路径', type: 'file', required: true }
    ],
    output: [
      { key: 'entries', label: '文件列表', type: 'json' }
    ],
  ...
  },
  {
    ...
    output: [
      { key: 'metadata', label: '视频信息', type: 'json' }
    ],
...
export const qiuaiToolActionIds = qiuaiToolActions.map((action) => action.actionId);

export function listToolActionsByPackageId(packageId: string): ToolActionDefinition[] {
  return qiuaiToolActions.filter((action) => action.packageId === packageId);
}

export function getToolActionDefinition(actionId: string): ToolActionDefinition | undefined {
  return qiuaiToolActions.find((action) => action.actionId === actionId);
}

export function getToolPackageDefinition(packageId: string): ToolPackageDefinition | undefined {
  return qiuaiToolPackages.find((toolPackage) => toolPackage.id === packageId);
}
